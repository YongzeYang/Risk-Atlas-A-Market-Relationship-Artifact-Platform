...
async function main() {
  ...
  console.log(
    `Prepared ${snapshotPlans.length} snapshot plans ` +
  ...
  for (let index = 0; index < snapshotPlans.length; index += 1) {
    ...
    console.log(
      `Running snapshot ${index + 1}/${snapshotPlans.length}: ` +
...
async function ensureHongKongDatasetReady(): Promise<DatasetCatalogRow> {
  ...
  if (current && universeReady) {
    console.log(
      `Hong Kong dataset already available: ${current.id} ` +
  ...
  if (!seeded || !(await hasUniverse(HK_UNIVERSE_ID))) {
    throw new Error(
      'Hong Kong dataset bootstrap finished but the HK dataset or hk_all_common_equity universe is still unavailable.'
    );
  }
...
async function ensureCryptoDatasetReady(): Promise<DatasetCatalogRow> {
  ...
  if (current && universeReady) {
    console.log(
      `Crypto dataset already available: ${current.id} ` +
        `(${current.catalogPriceRowCount.toString()} rows, asOf=${current.catalogMaxTradeDate ?? 'n/a'}).`
    );
    return current;
  }

  console.log('Crypto dataset or universe is missing. Running prisma/import-crypto-market-map.ts.');
  await runPrismaScript('prisma/import-crypto-market-map.ts', {
    CRYPTO_MARKET_MAP_SKIP_VERIFICATION_BUILD: '1'
  });
  ...
  if (!imported || !(await hasUniverse(CRYPTO_UNIVERSE_ID))) {
    throw new Error(
      'Crypto dataset bootstrap finished but crypto_market_map_yahoo_v2 or crypto_market_map_all is still unavailable.'
    );
  }
...
async function ensureSnapshotBuild(plan: SnapshotPlan): Promise<SnapshotBuildSummary> {
  ...
  if (existingSucceeded?.artifact) {
    console.log(
      `Reusing existing succeeded snapshot ${existingSucceeded.id} for ` +
        `${plan.marketLabel} ${plan.scoreMethod}.`
    );
  ...
  if (!universe) {
    throw new Error(`Universe "${plan.universeId}" was not found for snapshot bootstrap.`);
  }
  ...
  if (!completed || completed.status !== 'succeeded' || !completed.artifact) {
    throw new Error(
      `Snapshot build ${buildRun.id} failed for ${plan.marketLabel} ${plan.scoreMethod}: ` +
...
if (isMain) {
  main()
    .catch((error) => {
      console.error('Initial market bootstrap failed:', error);
      process.exitCode = 1;
    })
    .finally(async () => {
